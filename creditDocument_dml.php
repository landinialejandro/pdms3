...
function creditDocument_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$combo_dateIncomingNote->DateFormat = "ymd";
	...
	if($selected_id){
		...
		$templateCode = str_replace('<%%VALUE(dateIncomingNote)%%>', @date('Y-m-d', @strtotime(html_attr($row['dateIncomingNote']))), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(dateIncomingNote)%%>', urlencode(@date('Y-m-d', @strtotime(html_attr($urow['dateIncomingNote'])))), $templateCode);
