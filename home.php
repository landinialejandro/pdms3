...
<?php
	...
	if(is_array($arrTables) && count($arrTables)) {
		...
		foreach($tg as $tn => $tgroup){
			...
			$tChkHL = array_search($tn, array('ordersDetails','_resumeOrders','creditDocument','sogg_Terzi_Rapp_PA'));
			/* allow homepage 'add new' for current table? */
			$tChkAHAN = array_search($tn, array('orders','products','firstCashNote','vatRegister','companies','CLIENTI_CESSIONARI_PA','creditDocument','town','contacts_companies'));
